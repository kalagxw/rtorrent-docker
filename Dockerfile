...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get install -y wget libtorrent-dev rtorrent nginx-full unzip tzdata mediainfo \
...
RUN git clone https://github.com/kalagxw/rtorrent-docker.git a && cp ./a/flood-nginx /etc/nginx/sites-enabled/flood-nginx \
                   ...
                   && npm install jsonwebtoken && npm install && npm run build \
