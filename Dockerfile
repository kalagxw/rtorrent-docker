...
RUN apt-get update  && apt-get install -y git iputils-ping net-tools build-essential automake wget libcppunit-dev libtool zlib1g-dev gawk libsigc++-2.0-dev libncurses5-dev libncursesw5-dev libcurl4-openssl-dev libxmlrpc-c++8-dev unzip \
...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get install -y wget nginx-full php-fpm php7.2-cgi php7.2-cli php-geoip curl ffmpeg mediainfo unrar rar sox libsox-fmt-mp3 \
                   ...
                   && cd openssl && patch -p1 < ../openssl-patch/openssl-equal-3.0.0-dev_ciphers.patch && cd .. \
...
RUN rm -rf /etc/nginx/sites-enabled/default \
    ...
    && cd / && rm -rf rtorrent-0.9.7 rtorrent-0.9.7.tar.gz a openssl-patch openssl ngx_brotli nginx-patch nginx-ct.zip nginx-ct-1.3.2 nginx-1.15.8.tar.gz nginx-1.15.8 libtorrent
